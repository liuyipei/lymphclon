library(plyr)
#err2_mix_heatmap_4eq_logn: used in in paper figure; 4 similar ones with low noise
num.iterations <- 4
...
boxplot(err2 ~ power, data = err2.table, 
        ...
        main = "Simulated performance of baseline and our estimator (4 Similar Replicates on Low PCR Noise)",
...
plot(mean.err2.table$power, mean.err2.table[, paste(c(meth.to.plot, 'err2'), collapse = '.')], 
  ...
  main = sprintf('Clonality mean squared error compared over %d simulations (4 Similar Replicates on Low PCR Noise)', nrow(num.table)),
...
plot(mean.err2.table$power, mean.err2.table[, paste(c(abe.to.plot, 'abe2'), collapse = '.')], 
  col = 'blue', log = '', 
  main = sprintf('Abundance mean squared error compared over %d simulations (4 Similar Replicates on Low PCR Noise)', nrow(num.table)),
  pch = 2, ylim = ylim,
...
postscript('err2_mix_heatmap_4eq_logn.eps')
par(mar=c(5,4,4,2)+2) #should alleviate the margin problem (default is +0.1)
